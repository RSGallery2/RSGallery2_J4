...
class J3xImages2Dbtransfer extends Queue<Ij3xFile> {
...
class J3xGalleries extends Queue<Ij3xGallery> {

    addGalleries(galleries: Ij3xGallery[]) {

        for (let idx = 0; idx < galleries.length; idx++) {
            const gallery: Ij3xGallery = galleries[idx];
...
class FormElements {
    ...
    btnTransferByGallery: HTMLButtonElement;//
    //btnTransferByCheckedGalleries: HTMLButtonElement;
    ...
    constructor() {
        ...
        this.btnTransferByGallery = <HTMLButtonElement>document.getElementById('moveByGallery');
        // this.btnTransferByCheckedGalleries = <HTMLButtonElement>document.getElementById('moveByCheckedGalleries');
...
class GalleriesListTask {
    ...
    constructor(
        ...
        // RequestMoveFolderFilesTask: RequestMoveFolderFilesTask,
    ) {
...
//        formElements.btnMoveByGallery.onclick = onMoveByGallery();
        ...
        formElements.btnTransferByGallery.onclick = (ev) => this.onTransferByGallery(ev, formElements.selectGallery);
        //formElements.btnTransferByCheckedGalleries.onclick = (ev) => this.onTransferByCheckedGalleries(ev);
        formElements.btnMoveAllJ3xImjages.onclick = (ev) => this.onTransferAllGalleries(ev);
...
//    onMoveByGallery(ev: MouseEvent) {
    onTransferByGallery(ev: MouseEvent, selectGallery: HTMLSelectElement) {
    ...
    // onTransferByCheckedGalleries(ev: MouseEvent) {
    //     //let element = <HTMLInputElement>ev.target;
    //
    //     ev.preventDefault();
    //     ev.stopPropagation();
    ...
    onTransferAllGalleries(ev: MouseEvent) {
        let element = <HTMLInputElement>ev.target;
...
interface IResponseMove {
    ...
    state_thumb: string;
    state_watermarked: string;
    state_image_db: string;
...
//    private dragZone: HTMLElement;
    ...
    private j3xGalleries: J3xGalleries;
    private j3xImages2Move: J3xImages2Dbtransfer;
    ...
    constructor(
        ...
        j3xGalleries: J3xGalleries,
        j3xImages2Move: J3xImages2Dbtransfer,
    ...
    /**/
    private async callAjaxRequest(j3xGalleries: Ij3xGallery): Promise<any> {
...
function ajaxCaughtMessages2Html(errText: Error, title: string): HTMLElement | null {
            ...
            this.callAjaxMove(j3xImage)
                .then((response) => {
                    ...
                    if (AjaxResponse.success) {
                        ...
                        let moveData = <IResponseMove><unknown>AjaxResponse.data;
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    const j3xImages2Move = new J3xImages2Dbtransfer();
