...
//              $imgProperties = $imgSrcPath);
    ...
    public function deleteImgItemImages($imageFileName, $galleryId, $use_j3x_location)
    {
        ...
        try
        {
            ...
            foreach ($imagePathFileNames as $imageFileName)
            {
                ...
                // Make sure to not delete not existent
     ...
     * @since  5.1.0
     */
    public function MoveImageAndCreateRSG2Images($srcTempPathFileName, $targetFileName, $galleryId, $uploadOrigin, $use_j3x_location): array
    {
        global $rsgConfig, $Rsg2DebugActive;
...
//      if (false) {
        ...
        if ($Rsg2DebugActive)
        {
            Log::add('<== Exit MoveImageAndCreateRSG2Images: ' . (($isCreated) ? 'true' : 'false') . ' Msg: ' . $msg);
        }
    ...
    public function CreateRSG2Images(ImagePathsModel $imagePaths, $srcFileName = '', $targetFileName = ''): bool
    {
        ...
        if ($Rsg2DebugActive)
        {
            Log::add('<== Exit CreateRSG2Images: ' . (($isCreated) ? 'true' : 'false'));
        }
     ...
     * @since  5.1.0
     */
    public function CreateRSG2ImagesJ3x(ImagePathsJ3xModel $imagePaths, $srcFileName = '', $targetFileName = ''): bool
    {
        global $rsgConfig, $Rsg2DebugActive;
        ...
        if (file_exists($srcFileName))
        {
            ...
            if ($isCreated)
            {
                ...
                try
                {
                    ...
                    $isCreated = $this->createDisplayImageFile($imagePaths->getDisplayPath($targetFileName), $imageSize, $memImage);
        ...
        if ($Rsg2DebugActive)
        {
            Log::add('<== Exit CreateRSG2ImagesJ3x: ' . (($isCreated) ? 'true' : 'false'));
        }
    ...
    private function use_j3x_location($ImageId)
    {
        ...
        {
            $db    = $this->getDatabase();
            $query = $db->createQuery()->select($db->quoteName('use_j3x_location'))->from($db->quoteName('#__rsg2_images'))->where($db->quoteName('id') . ' = ' . $db->quote($ImageId));
            $db->setQuery($query);
    ...
    public function imageFileAttrib($ImageId)
    {
        ...
        try
        {
            ...
            $query = $db->createQuery()->select($db->quoteName(['name', 'gallery_id', 'use_j3x_location']))->from($db->quoteName('#__rsg2_images'))->where($db->quoteName('id') . ' = ' . $db->quote($ImageId));
            $db->setQuery($query);
