...
function markImages_nGalleryTimes(maxGalleries) {
    ...
    j3x_rows = Array.from(document.getElementsByName("j3x_gal_row"));
    // all gallery rows
    j3x_form = document.getElementById('adminForm');
    // boxchecked = <HTMLInputElement>j3x_form.querySelector('input[name="boxchecked"]');
    boxchecked = document.getElementsByName('boxchecked')[0];
    j3x_rows.forEach((j3x_row) => {
        ...
        if (galleries.length < maxGalleries) {
            ...
            if (!isMerged) {
                ...
                gallery_checkbox = j3x_row.querySelector('input[type="checkbox"]');
